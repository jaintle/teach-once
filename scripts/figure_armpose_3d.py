#!/usr/bin/env python
"""Figure: arm-pose 3-D generalisation experiment — Phase 9.

Runs the arm-pose analogue of the GPT generalisation experiment in 3-D:

  (a) Source and transported demo trajectory pair.
  (b) Success rate bar chart across randomised arm configurations.
  (c) Final error box plot across trials.

Saves to ``reports/figures/phase9_armpose_3d.png`` (or ``--out_dir``).

Usage
-----
    python scripts/figure_armpose_3d.py --seed 0 --n_trials 10
"""
...
from gpt_repro.policies.demos_3d import make_armpose_demo, randomize_armpose_scene
...
from gpt_repro.viz.viz_3d import plot_generalization_trials
...
def main(args: argparse.Namespace) -> None:
    ...
    # -- Base demo and scene ------------------------------------------------
    demo, scene = make_armpose_demo(seed=args.seed)

    # -- Transport base demo (identity S == T) ------------------------------
    env_base = ArmPoseEnv(scene=scene)
    ...
    gen_result = evaluate_generalization_3d(
        base_demo=demo,
        base_scene=scene,
        randomize_fn=randomize_armpose_scene,
    ...
    print(f"ArmPose success rate : {gen_result['success_rate']:.2f}")
    print(f"Mean final error     : {gen_result['mean_error']:.4f} m")

    # -- Figure ---------------------------------------------------------------
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))

    # Panel (a): 3-D trajectory
    ax_traj = axes[0]
    ax_traj.remove()
    ...
    fig.suptitle(f"Phase 9: ArmPose 3D generalisation (seed={args.seed})", fontsize=12)
    fig.tight_layout()

    out_path = out_dir / "phase9_armpose_3d.png"
    fig.savefig(out_path, dpi=150)
    plt.close(fig)

    traj_out = out_dir / "phase9_armpose_traj.png"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Phase 9 arm-pose 3D figure")
