"""
Animate reshelving policy-transport rollouts (Phase 12).

Generates a 2×2 tiled GIF (and MP4 if imageio-ffmpeg is installed) showing
GP-transported rollouts across N target scenes.

CLI
---
  python scripts/animate_reshelving.py [--seed 0] [--n_scenes 4] [--fps 15]
...
def _tile_2x2(frames_per_env: list[list[np.ndarray]]) -> list[np.ndarray]:
    ...
    h, w = frames_per_env[0][0].shape[:2]
    tiled = []
    for i in range(n):
...
def _collect_rollout_frames(
    ...
    # Reset to the trajectory start position
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Animate reshelving rollouts")
    ...
    # ── 1. Build source demo ──────────────────────────────────────────────────
    demo, source_scene = make_reshelving_demo(seed=args.seed)
    ...
    for i in range(scenes_needed):
        scene = randomize_reshelving_scene(
            seed=args.seed + i + 1,
            base_scene=source_scene,
        )
        env = ReshelvingEnv(scene=scene, render_mode="rgb_array")
    ...
    # Use first 4 scenes for 2×2 tile
    tile_frames = _tile_2x2(all_frames[:4])
    # Subsample to keep GIF small (every 3rd frame)
    gif_frames = tile_frames[::3][:60]

    total_frames = len(gif_frames)
    print(f"Total tiled frames: {total_frames}")

    # ── 3. Save GIF ───────────────────────────────────────────────────────────
    gif_path = out_dir / "reshelving_rollout.gif"
    ...
    # Downscale if > 8 MB
    if gif_size_kb > 8192:
        print("GIF > 8MB — downscaling to 480×480 total ...")
        small = [
            np.array(
                imageio.v2.imresize(f, (480, 480))  # type: ignore[attr-defined]
                if hasattr(imageio.v2, "imresize")
                else f[::2, ::2]
            )
            for f in gif_frames
        ]
    ...
    # ── 4. Save MP4 (optional) ────────────────────────────────────────────────
    mp4_path = out_dir / "reshelving_rollout.mp4"
    try:
